...
void deleteToStorageC(int id){
	FILE *fichier_clients = fopen("client.dat", "r+b");	//ouverture en lecture/écriture du fichier clients
    ...
    Header header;
    fread(&header, sizeof(Header), 1, fichier_clients);	
	Client clients[header.nbRecords];
	
	if (delete_client(clients,id)) {
		fseek(fichier_clients, header.lengthHeader+(id-1)*header.lengthRecord, SEEK_SET);
		fwrite(&clients[id-1],header.lengthRecord,1,fichier_clients);
		fclose(fichier_clients);
...
void deleteToStorageT(int id_transaction) {
    FILE *fichier_transactions = fopen("transaction.dat", "r+b"); // ouverture en lecture/écriture
    if (!fichier_transactions) {
        perror("Erreur lors de l'ouverture du fichier transactions");
        return;
    }
    ...
    Transaction transactions[header.nbRecords]; // Modification pour charger toutes les transactions
    if (delete_transaction(transactions, id_transaction)) {
        fseek(fichier_transactions, header.lengthHeader + (id_transaction-1) * header.lengthRecord, SEEK_SET);
        fwrite(&transactions[id_transaction - 1], header.lengthRecord, 1, fichier_transactions); // Modification pour utiliser id_transaction - 1
        fclose(fichier_transactions);
...
void deleteToStorageV(const char *immatriculation) {
    Voiture *voitures;
    int size;
    voitures = charger_voitures(&size); // Chargement des données pour déterminer la position
    ...
    if (delete_voiture(&voitures[position], immatriculation)) {
        FILE *fichier_voitures = fopen("voiture.dat", "r+b"); // ouverture en lecture/écriture
        if (!fichier_voitures) {
            perror("Erreur lors de l'ouverture du fichier voitures");
            return;
        }
        ...
        fseek(fichier_voitures, header.lengthHeader + (position-1) * header.lengthRecord, SEEK_SET);
        fwrite(&voitures[position], header.lengthRecord, 1, fichier_voitures);
        fclose(fichier_voitures);
